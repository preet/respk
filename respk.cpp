...
std::string const g_resgen_file_header =
                                ...
                                std::string &output)
{
    ...
    if(!res_file) {
        ...
        if(!res_file) {
            std::cout << "resgen: ERROR: Could not open resource file: " << std::endl;
...
bool GenerateSourceFromResourceDesc(std::string const &output_path,
                                    std::string const &input_path)
{
    std::ifstream input_file(input_path);
    if(!input_file) {
        std::cout << "resgen: ERROR: Could not open file "
                  << input_path << std::endl;
        return false;
    }
    ...
    if(!err.empty()) {
        std::cout << "resgen: ERROR: Could not parse json: " << std::endl;
    ...
    if(!json_root_val.is<picojson::object>()) {
        std::cout << "resgen: ERROR: JSON is not an object " << std::endl;
    ...
    auto file_it = root_obj.find("file");
    if(file_it == root_obj.end()) {
        std::cout << "resgen: ERROR: json does not contain "
                     "a \"file\" entry" << std::endl;
        return false;
    }
    else {
        if(!file_it->second.is<std::string>()) {
            std::cout << "resgen: ERROR: file value must be a string" << std::endl;
            return false;
        }
        else {
            cpp_file_path =
                    output_path +
                    file_it->second.get<std::string>();
        }
    }
    ...
    if(resources_it == root_obj.end()) {
        ...
        if(!resources_it->second.is<picojson::array>()) {
            std::cout << "resgen: ERROR: resources value "
                ...
                ++res_value_it)
            {
                ...
                if(!res_value_it->is<picojson::object>()) {
                    std::cout << "resgen: ERROR: resource must "
                    ...
                    if(res_name_it == res_obj.end()) {
                        std::cout << "resgen: ERROR: resource " << res_idx
                    ...
                    if(!res_name_it->second.is<std::string>()) {
                        std::cout << "resgen: ERROR: resource name " << res_idx
                                  << " must be a string" << std::endl;
                        return false;
                    }
                    if(!res_path_it->second.is<std::string>()) {
                        std::cout << "resgen: ERROR: resource path " << res_idx
                                  << " must be a string" << std::endl;
                        return false;
                    }
    ...
    if(prepend_it != root_obj.end()) {
        if(!prepend_it->second.is<std::string>()) {
            std::cout << "resgen: ERROR: prepend value must be a string" << std::endl;
            return false;
        }
        else {
    ...
    if(append_it != root_obj.end()) {
        if(!append_it->second.is<std::string>()) {
            std::cout << "resgen: ERROR: append value must be a string" << std::endl;
            return false;
        }
        else {
    ...
    // save data to file
    std::string cpp_file_str;
    cpp_file_str.append(g_resgen_file_header);
    cpp_file_str.append(cpp_prepend);
    cpp_file_str.append("\n");
    ...
    for(auto & res : cpp_list_resources) {
        ...
        cpp_file_str.append(res_data);
    }
    cpp_file_str.append("\n\n");
    cpp_file_str.append(cpp_append);
    cpp_file_str.append("\n");


    std::ofstream cpp_file;
    cpp_file.open(cpp_file_path);
    if(!cpp_file) {
        std::cout << "resgen: ERROR: failed to open output file: " << std::endl;
        std::cout << cpp_file_path << std::endl;
        return false;
    }

    cpp_file << cpp_file_str;
    cpp_file.close();
...
int main(int argc, char *argv[])
{
    ...
    for(auto const &input : list_inputs) {
        if(!GenerateSourceFromResourceDesc(output,input)) {
            return false;
        }
    }
